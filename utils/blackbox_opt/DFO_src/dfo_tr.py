...
class result:
    def __init__(self, x, f, iteration, iter_suc, func_eval, delta, x_data, y_data):
        self.x = x
        self.fun = f
        ...
        self.delta = delta
        # own modif - log evaluated points
        self.x_data = x_data
        self.y_data = y_data
...
def _set_parameters(options):
    ...
    if options:
        for key in options:
            if (key not in list(par.__dict__.keys()) and
...
def _build_initial_sample(x_initial, delta, options=None):
    ...
    # set it to auto.
    if not options or "sample_gen" not in list(options.keys()):
        option = "auto"
    else:
...
def dfo_tr(bb_func, x_initial, options=None):
    ...
    the optimization problem. The main functions inside this function are
    ...
    while True:
        ...
        if normg <= tol_norm_g: 
            print('Stopping criterion: norm of gradient g={0}<gtol={1}'
                .format(normg, tol_norm_g))
            break
        if delta < tol_delta:
            print('Stopping criterion: TR radius delta={0}<deltatol={1}'
                .format(delta, tol_delta))
            break
        ...
        fmod = f + val
        # NOTE from me
        # this does not look ok but this is ok
        # because M(x)=f(xc)+m(x)=f(xc)+gT(x-xc)+0.5(x-xc)TH(x-xc)
        # so M(xc)=f(xc)
        # and M(xc+s)=f(xc)+val

        # Stop if the reduction is very small or max iterations reached
        if iteration > max_iter:
            print('Stopping criterion: max iterations reached {0}'.format(max_iter))
            break
        if abs(val) < tol_f:
            print('Stopping criterion: decrease in f: {0}<{1}'.format(val, tol_f))
            break
        ...
        # TODO log
        x_data = np.hstack((x_data, xtrial))
        y_data = np.vstack((y_data, ftrue))
    ...
    res = result(x, f, iteration, iter_suc, func_eval, delta, x_data=x_data, y_data=y_data)
    return res
