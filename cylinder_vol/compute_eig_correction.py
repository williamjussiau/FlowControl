...
if __name__=='__main__':
            ...
            for i, fi in enumerate(ff[ff<=Fs_sn/2]):
                ...
                if write_fft_xdmf:
                    for op, vec, name in zip([np.abs, np.real, np.imag], 
                        ...
                        flu.write_xdmf(resultpath + name + '.xdmf', vec, name, time_step=fi, append=append)
                        append=True
    ...
    flu.export_to_mat(fs.savedir0 + 'data/Q.npz', fs.savedir0 + 'data/Q.mat', 'Q')
    ...
    #Q_sp = flu.spr.load_npz(fs.savedir0 + 'data/Q.npz') 
    ...
    # Sanity check
    erk = np.linalg.norm(A_mean_sp@rk - muk*Q_sp@rk)
    elk = np.linalg.norm(herm(A_mean_sp)@lk - mukstar*Q_sp@lk)
    print('Error on (rk, lk, muk) is: ', erk, elk)

    # Normalize wrt Q-seminorm and stuff
    Qnorm = Q_sp # is norm Quvp=Quvp_sp or Quv=Q_sp?
    ...
    for ip, p in enumerate(plist):
        ...
        Mp = (muk + p*1j*w0)*Q_sp - A_mean_sp
        Mmp = (muk - p*1j*w0)*Q_sp - A_mean_sp
        ...
        Jmp_re_sp = Jp_re_sp
        Jmp_im_sp = flu.dense_to_sparse(Jmp_im) # result is not exactly the same with -Jmp_im
        Jmp_sp = Jmp_re_sp + 1j*Jmp_im_sp
        ...
        # skp_ip = (spsolve(Mp.T, herm(Jp_sp) @ Q_sp.T @ lk_normed.conj())).T @ Jp_sp @ rk_normed 
        # skmp_ip = (spsolve(Mmp.T, herm(Jmp_sp) @ Q_sp.T @ lk_normed.conj())).T @ Jmp_sp @ rk_normed 
    ...
    print('*** -p: ', skmp[:,None])
    print('*** Sum: ', sk2)
    print('muk: ', muk.reshape(1,))
    print('corr:', muk+np.sum(sk2))
    ...
    #    #flu.spr.save_npz(resultpath + 'data/Q.npz', Q_sp)
