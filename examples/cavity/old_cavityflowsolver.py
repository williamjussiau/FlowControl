...
class CavityFlowSolver(FlowSolver):
    ...
    def define_paths(self):
        ...
        file_extension = (
            ""
            if Tstart == 0
            else "_restart" + str(np.round(Tstart, decimals=3)).replace(".", ",")
        )
        ...
        filename_timeseries = (
            savedir0 + "timeseries1D" + file_extension_csv
        )  # 1D data as pd.DataFrame
        # Note:
    ...
    def make_boundaries(self):
        ...
        class bnd_lower_wall_right_sf(SubDomain):
            ...
            def inside(self, x, on_boundary):
                return (
                    on_boundary and between(x[0], (x0ns_right, xinf)) and near(x[1], 0)
                )
    ...
    def make_sensor(self):
        ...
        xs1 = 1.1
        MESH_TOL = DOLFIN_EPS
        sensor_subdm = CompiledSubDomain(
    ...
    def make_bcs(self):
        ...
        boundaries_idx = [
            ...
            CAVITY_BOTM_IDX,
            CAVITY_RIGHT_IDX,
            LOWER_WALL_LEFT_SF_IDX,
            LOWER_WALL_LEFT_NS_IDX,
            LOWER_WALL_RIGHT_NS_IDX,
            LOWER_WALL_RIGHT_SF_IDX,
        ]
        ...
        bcp_outlet = DirichletBC(
            self.W.sub(1), Constant(0), self.boundaries.loc["outlet"].subdomain
        )
        bcp = [bcp_outlet]
        ...
        # inlet : u=uinf, v=0
        bcu_inlet = DirichletBC(
            self.W.sub(0), Constant((0, 0)), self.boundaries.loc["inlet"].subdomain
        )
    ...
    def load_steady_state(self, assign=True):
        ...
        if assign:
            ...
            self.Eb = self.compute_energy(
                field=u0, full=True, diff=True, normalize=False
            )
            ...
            self.Eb_r = self.compute_energy(
                field=u0, full=False, diff=True, normalize=False
            )
    ...
    def compute_steady_state(self, method="newton", u_ctrl=0.0, **kwargs):
        ...
        self.Eb = (
            1 / 2 * norm(u0, norm_type="L2", mesh=self.mesh) ** 2
        )  # same as <up, Q@up>
        # Eb restricted
        self.u_restrict.vector()[:] = self.u0.vector()[:] * self.IF.vector()[:]
        self.Eb_r = (
            1 / 2 * norm(self.u_restrict, norm_type="L2", mesh=self.mesh) ** 2
    ...
    def compute_steady_state_picard(self, max_iter=10, tol=1e-14):
        ...
        Lp = (
            dot(f, v) * dx + Constant(0) * inner(u0, v) * dx + Constant(0) * q * dx
        )  # rhs = actuation
        bp = assemble(Lp)
    ...
    def init_time_stepping(self):
        ...
        # inlet : u=uinf, v=0
        bcu_inlet_g = DirichletBC(
            V, Constant((self.uinf, 0)), self.boundaries.loc["inlet"].subdomain
        )
        # upper wall : v=0 + dy(u)=0 (done in weak form)
        bcu_upper_wall_g = DirichletBC(
            V.sub(1), Constant(0), self.boundaries.loc["upper_wall"].subdomain
        )
        # lower wall left sf : v=0 + dy(u)=0
        bcu_lower_wall_left_sf_g = DirichletBC(
            V.sub(1), Constant(0), self.boundaries.loc["lower_wall_left_sf"].subdomain
        )
        # lower wall left ns : u=0; v=0
        bcu_lower_wall_left_ns_g = DirichletBC(
            V, Constant((0, 0)), self.boundaries.loc["lower_wall_left_ns"].subdomain
        )
        # lower wall right ns : u=0; v=0
        bcu_lower_wall_right_ns_g = DirichletBC(
            V, Constant((0, 0)), self.boundaries.loc["lower_wall_right_ns"].subdomain
        )
        # lower wall right sf : v=0 + dy(u)=0
        bcu_lower_wall_right_sf_g = DirichletBC(
            V.sub(1), Constant(0), self.boundaries.loc["lower_wall_right_sf"].subdomain
        )
        # cavity : no slip, u=0; v=0
        bcu_cavity_left_g = DirichletBC(
            V, Constant((0, 0)), self.boundaries.loc["cavity_left"].subdomain
        )
        bcu_cavity_botm_g = DirichletBC(
            V, Constant((0, 0)), self.boundaries.loc["cavity_botm"].subdomain
        )
        bcu_cavity_right_g = DirichletBC(
            V, Constant((0, 0)), self.boundaries.loc["cavity_right"].subdomain
        )
        bcug = [
            bcu_inlet_g,
            bcu_upper_wall_g,
            bcu_lower_wall_left_sf_g,
            bcu_lower_wall_left_ns_g,
            bcu_lower_wall_right_ns_g,
            bcu_lower_wall_right_sf_g,
            bcu_cavity_left_g,
            bcu_cavity_botm_g,
            bcu_cavity_right_g,
        ]

        bcp_outlet_g = DirichletBC(
            P, Constant(0), self.boundaries.loc["outlet"].subdomain
        )
        bcpg = [bcp_outlet_g]
        ...
        sysAssmb1 = [
            SystemAssembler(a1g, L1g, self.bc_ipcs["bcu"]),
        ...
        sysAssmb2 = [
            SystemAssembler(a1g2, L1g2, self.bc_ipcs["bcu"]),
        ...
        colnames = (
            ["time", "u_ctrl"]
            + y_meas_str
            + ["u_norm", "p_norm", "dE", "cl", "cd", "runtime"]
        )
...
if __name__ == "__main__":
    ...
    print("Trying to instantiate CavityFlowSolver...")
    params_flow = {
        "Re": 7500.0,
        "uinf": 1.0,
        "d": 1.0,
