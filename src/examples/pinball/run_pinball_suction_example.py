"""
Fluidic pinball — suction actuation
3 cylinders, nondimensional incompressible Navier-Stokes.
Suggested Re=100.

Demonstrates:
- PinballCustomInitialGuess for Picard convergence
- compute_steady_state with force-coefficient reporting
- open-loop Gaussian-bump actuation on 3 suction actuators
"""
...
def main():
    ...
    actuator_mid = ActuatorBCParabolicV(width=actuator_width, position_x=position_mid[0], boundary_name="actuator_mid")
    actuator_top = ActuatorBCParabolicV(width=actuator_width, position_x=position_top[0], boundary_name="actuator_top")
    actuator_bot = ActuatorBCParabolicV(width=actuator_width, position_x=position_top[0], boundary_name="actuator_bot")
    ...
    params_control = flowsolverparameters.ParamControl(
        ...
        user_data={"mode_actuation": CYLINDER_ACTUATION_MODE.SUCTION},
    )
    ...
    # Open-loop: Gaussian-bump excitation on each actuator
    tlen = 0.10
    tpeak = [0.25, 0.5, 0.75]
    u0peak = [+2.0, -1.5, -2.0]
