...
if __name__ == "__main__":
    ...
    if COMPUTE_OPERATORS_CAVITY:
        fs = CavityFlowSolver.make_default(Re=7500, path_out=cwd / "cavity" / "data_output")
    ...
    if COMPUTE_OPERATORS_LIDCAVITY:
        fs = LidCavityFlowSolver.make_default(Re=8000, path_out=cwd / "lidcavity" / "data_output")
        fs.load_steady_state()
        compute_operators_flowsolver(fs, export=True)

    if COMPUTE_OPERATORS_PINBALL:
        fs = PinballFlowSolver.make_default(Re=50, path_out=cwd / "pinball" / "data_output")
