...
def export_flowsolver_matrices(fs, path, suffix=""):
    ...
    for mat, matname in zip([A, B, C, Q], ["A", "B", "C", "Q"]):
        # Convert to sparse
        mat_spr = dense_to_sparse(
            mat, eliminate_zeros=matname == "B", eliminate_under=1e-14
        )
...
if __name__ == "__main__":
    ...
    if 1:
        logger.info("")
        tc = time.time()
        AA, BB = make_mat_to_test_slepc(
            view=True, singular=True, neigpairs=2000, density_B=7 / 10, rand=False
        )
        ...
        if 1:
            ...
            for pc_key, pc_name in all_pc.items():
                try:
                    t0 = time.time()
                    eigz, eigensolver = get_mat_vp_slepc(
                        array_to_petscmat(AA),
                        array_to_petscmat(BB),
                        target=-0.1,
                        n=4,
                        return_eigensolver=True,
                        precond_type=pc_name,
                        verbose=True,
                    )
                    tpc = time.time() - t0
