...
if __name__ == "__main__":
    ...
    if use_smt:
        ...
        costfun_parallel_smt = lambda x: parallel_function_wrapper(
            x, [0], costfun_array
        )
        if rank == 0:
            stop = [0]
        ...
        costfun_parallel_scp = lambda x: parallel_function_wrapper(
            x, [0], costfun, allout=False
        )
        if rank == 0:
            stop = [0]
            x = np.zeros(1)
