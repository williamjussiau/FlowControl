...
def controller_residues_wrapper(theta, n_real, n_cplx):
    ...
    (
        ...
        cplx_p_re_idx,
        cplx_p_im_idx,
    ) = controller_residues_getidx(n_real, n_cplx)
...
def test_controller_residues(n_real, n_cplx):
    ...
    # K1: vanilla formula
    K1 = controller_residues(
        real_c, real_p, cplx_c_re + 1j * cplx_c_im, cplx_p_re + 1j * cplx_p_im
    )
...
def balred_rel(sys, hsv_threshold, method="truncate"):
    ...
    if method != "truncate" and method != "matchdc":
        ...
        try:
            ...
            raise Exception("can't find slycot subroutine ab09md or ab09ad")
    elif method == "matchdc":
        try:
    ...
    alpha = 0.0  # redefine stability bnd
    ...
    if method == "truncate":
        ...
        if np.any(np.linalg.eigvals(sys.A).real >= 0.0):
            ...
            Nr, Ar, Br, Cr, _ = ab09ad(
                time_type, job, equil, n, m, p, sys.A, sys.B, sys.C, nr=nr, tol=tol
            )
...
def norm(G, p=np.inf, hinf_tol=1e-6, eig_tol=1e-8):
    ...
    if p not in (2, np.inf):
        raise ValueError(
            "The p in p-norm is not 2 or infinity. If you"
...
if __name__ == "__main__":
    ...
    if 1:
        ...
        P_4blk = (
            Wout
            * ss_vstack(
                ss_hstack(Iss, -Iss, Zss, Zss),
                ss_hstack(Zss, Zss, Iss, Zss),
                ss_hstack(Zss, Iss, Zss, -Iss),
                ss_hstack(Iss, -Iss, Zss, Zss),
            )
            * ss_blkdiag_list([Iss, G, Iss, CLr])
            * ss_vstack(
                ss_hstack(Iss, Zss, Zss),
                ss_hstack(Zss, Iss, Iss),
                ss_hstack(Zss, Zss, Iss),
                ss_hstack(Zss, Iss, Zss),
            )
            * Win
        )
