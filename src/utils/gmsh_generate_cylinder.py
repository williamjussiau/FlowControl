...
class MeshGenerator:
    ...
    def set_mesh_param(self, default=False, **mesh_param):
        ...
        Mesh topology is the following:
                 ...
                 |         -------------------------        |
                 -------- -----------------------------------
        Additionally, the cylinder O is made of 'segments' elements (default: 1/° >> 360)
    ...
    def build_mesh(self):
        ...
        # set mesh size
        ext_ = gmsh.model.getEntitiesInBoundingBox(
            prm["xinfa"], -prm["yinf"], -1, prm["xinf"], prm["yinf"], 1, dim=0
        )
        mid_ = gmsh.model.getEntitiesInBoundingBox(
            prm["xinfa"], -prm["yint"], -1, prm["xinf"], prm["yint"], 1, dim=0
        )
        in_ = gmsh.model.getEntitiesInBoundingBox(
            prm["xinfa"], -prm["lint"], -1, prm["xinf"], prm["lint"], 1, dim=0
        )
        ...
        gmsh.model.mesh.setSize(
            cyl_, np.min([1 / prm["n1"], 2 * np.pi / prm["segments"]])
        )
