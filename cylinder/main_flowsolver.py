...
class FlowSolver:
    ...
    def make_actuator(self):
        ...
        # actuator_bc = dolfin.Expression(['0','(x[0]>=L || x[0] <=-L) ? 0 : ampl*exp(-0.5 * x[0]*x[0] / den)'],
        # actuator_bc = dolfin.Expression(['0', '(x[0]>=L || x[0] <=-L) ? 0 : \
    ...
    def compute_steady_state(self, method="newton", u_ctrl=0.0, **kwargs):
        ...
        if self.Tstart == 0:  # and compute_steady_state
            ...
            # fa = dolfin.FunctionAssigner(receiving_fspace, assigning_fspace)
