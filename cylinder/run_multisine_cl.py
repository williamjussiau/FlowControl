...
def main(argv):
    ...
    if is_test:
        ...
        savedir0 = '/scratchm/wjussiau/fenics-results/cylinder_o1_ms_cl_G8_{0}/'.format(k_arg)
    ...
    params_time = {'dt': 0.005, # not recommended to modify dt between runs 
                   ...
                   'Tstart': 2500, # start of this sim # TODO # 1st switching/stacking iter is 1000
                   'Trestartfrom': 2000, # last restart file # TODO
                   'num_steps': 100000, #  
                   'Tc': 0} 
    ...
    # identified system
    #sspath = '/scratchm/wjussiau/fenics-python/cylinder/data/o1/lco/iter1/'
    ...
    #sspath = '/scratchm/wjussiau/fenics-python/cylinder/data/o1/lco/tcfd/iter3/' # TODO
    sspath = '/scratchm/wjussiau/fenics-python/cylinder/data/o1/lco/tcfd/iter5/' # TODO


    # iterations
    # from old_iterations > working
    #  1  2  3  4...
    # model order for working (stacking) iterations:
    # [8, 8, 6, 6, 8, 8, 8, 8, 8]
    # starting times for working (stacking) iterations:
    ...
    ########### MODEL
    # TODO
    #Glist = [10, 12, 14, 16] # available orders for G
    #G = flu.read_ss(sspath + 'Gi' + str(Glist[k_arg-1]) + '.mat')
    #G = flu.read_ss(sspath + 'G14.mat')
    #G = flu.read_ss(sspath + 'G8.mat')
    G = flu.read_ss(sspath + 'G8A0001.mat')
    #G = flu.read_ss(sspath + 'Gi14.mat') # order 14
    ...
    #Qu = 10**(Qulist[k_arg-1]) # Qu low = low penalty on u = big u  = high feedback gain
    #Qv = 10**(Qvlist[k_arg-1]) # Qv low = low noise on y = follow y = high observation gain 
    Qu = 10**(3)
    Qv = 10**(-1)
    
    # it3: G8, LQG 3, -2
    # it5: approx G8, LQG 3, -1?
    ...
    x_ctrl_stack = np.hstack((x_ctrl0, np.zeros(G.nstates)))
    ...
    ##### not time-based
    lstsq = lambda A, b: np.asarray(np.linalg.lstsq(A, b, rcond=None)[0])
    # u1=u2 (lsqminnorm)
    Au1u2 = Kss0.C
    bu1u2 = Kss00.C @ x_ctrl0 # this is supposedly u_ctrl_1[Tend]
    x_ctrl_u = lstsq(Au1u2, bu1u2) 
    # du1=du2 (lsqminnorm)
    yend = fs.y_meas[2] # should take y_meas_previous[2] but not available
    Adu1du2 = np.vstack((Kss0.C, Kss0.C@Kss0.A))
    bdu1du2 = np.vstack((Kss00.C, Kss00.C@Kss00.A))@x_ctrl0.reshape(-1,1) + \
        np.vstack((0, Kss00.C@Kss00.B - Kss0.C@Kss0.B))*yend
    x_ctrl_du = lstsq(Adu1du2, bdu1du2)
    # bumpless by safonov: limit of u1=u2 for Tlim=0: slow=all, fast={}
    Tlim = 0.2 # cut poles under Tlim response time
    wlim = 2*np.pi/Tlim # cut poles under wlim pulsation
    Kslow, Kfast = yu.slowfast(Kss0, wlim) # separate new controller
    ...
    x_ctrl_sf = np.hstack((x_ctrl_fast, x_ctrl_slow))
    ...
    filename_timeseries_prev = savedir0 + 'timeseries1D{0}.csv'.format(restart_extension)
    ...
    x_ctrl_r_list = []
    for rr in [5, 10, 20]:
        ur = np.asarray(timeseries_prev.u_ctrl[-rr-1:-1]) # shift -1 or not?
        yr = np.asarray(timeseries_prev.y_meas_3[-rr:]) # shift -1 or not?
        xn, yhat, uhat = yu.condswitch(ur=ur, yr=yr, K=Kss, dt=fs.dt, w_u=1.0, w_y=1.0, w_decay=0.8)
        x_ctrl_r_list.append(xn)
    ...
    # *** define state here
    # choose method
    #x_ctrl_list = [x_ctrl_stack, \
    #    x_ctrl_u, \
    #    x_ctrl_r, \
    #    x_ctrl_sf]
    x_ctrl_list = [x_ctrl_stack] + x_ctrl_r_list
    if k_arg<=4:
        x_ctrl_Kss0 = x_ctrl_list[k_arg-1]
    #if k_arg==4:
    #    Kss0 = Kfast + Kslow # such that K.A=diag(Aslow, Afast) IN THIS ORDER (predictable?)
    #else:
    #    x_ctrl_Kss0 = np.zeros(Kss.nstates,)
    #x_ctrl = x_ctrl_list[k_arg] # 0, 1, u, du
    ...
    #Kss0 = Kss
    #Kss1 = Kssred
    x_ctrl0 = x_ctrl_Kss0.reshape(-1,) 
    x_ctrl1 = x_ctrl_Kss1.reshape(-1,)
